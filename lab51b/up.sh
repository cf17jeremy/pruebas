...
docker-compose exec apache /usr/sbin/a2ensite  prova1.conf
docker-compose exec apache /usr/sbin/a2ensite  prova2.conf
docker-compose exec apache /usr/sbin/a2ensite  prova3.conf

docker-compose exec apache2 /usr/sbin/a2ensite  prova4.conf
docker-compose exec apache2 /usr/sbin/a2ensite  prova5.conf
docker-compose exec apache2 /usr/sbin/a2ensite  prova6.conf
...
# accedint per nom
NOM_SERVER=www.prova2.com
echo -e "\n${yellow}Validació: comprovació des del client (per nom - $NOM_SERVER ) ${endColor}\n"
docker-compose exec textclient curl  $PROXY http://$NOM_SERVER

# accedint per nom
NOM_SERVER=www.prova3.com
echo -e "\n${yellow}Validació: comprovació des del client (per nom - $NOM_SERVER ) ${endColor}\n"
docker-compose exec textclient curl  $PROXY http://$NOM_SERVER

# accedint per nom
NOM_SERVER=www.prova4.com
echo -e "\n\n${yellow}Validació: comprovació des del client (per nom - $NOM_SERVER ) ${endColor}\n"
docker-compose exec textclient curl  $PROXY http://$NOM_SERVER

# accedint per nom
NOM_SERVER=www.prova5.com
echo -e "\n\n${yellow}Validació: comprovació des del client (per nom - $NOM_SERVER ) ${endColor}\n"
docker-compose exec textclient curl  $PROXY http://$NOM_SERVER

# accedint per nom
NOM_SERVER=www.prova6.com
echo -e "\n\n${yellow}Validació: comprovació des del client (per nom - $NOM_SERVER ) ${endColor}\n"
docker-compose exec textclient curl  $PROXY http://$NOM_SERVER
